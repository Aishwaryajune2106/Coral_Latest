...
const VerifyPinEmail = ({navigation}) => {
  ...
  const handleGetOtp = () => {
    if (otp.every(digit => digit !== '')) {
      Alert.alert('OTP Submitted', `Your OTP: ${otp.join('')}`);
  ...
  const handleResendCode = () => {
    ...
    // Hide the resend message after 5 seconds
    setTimeout(() => setResendMessageVisible(false), 5000);

    // Enable the "Resend Code" link after 30 seconds
